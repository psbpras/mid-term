...
def test_repl_execution(_, capsys):
    ...
    assert (
        "===== Advanced Python Calculator =====" in captured.out
    ...
    assert (
        "Calculation History:" in captured.out
    ), "Expected history section in output."
    assert (
        "2.0 + 3.0 = 5.0" in captured.out
    ), "Expected '2.0 + 3.0 = 5.0' in history output."

    # Check exit message
    assert (
        "Exiting the calculator. Goodbye!" in captured.out
    ), "Expected exit message in output."
